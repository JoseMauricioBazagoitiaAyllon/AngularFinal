...
export class EmpleadoService {
  ...
  getById(CodEmp:number):Observable<Emp>{
    return this.http.get<any>(`${environment.API_URL}/api/Empleados/${CodEmp}`)
    .pipe(catchError(this.handlerError));
  }
  new(Emp:Emp):Observable<Emp>{
    return this.http.post<Emp>(`${environment.API_URL}/api/Empleados`,Emp)
    .pipe(catchError(this.handlerError));
  }
  update(CodEmp:number, Emp:Emp ):Observable<Emp>{
    return this.http.patch<Emp>(`${environment.API_URL}/api/Empleados/${CodEmp}`,Emp)
    .pipe(catchError(this.handlerError));
  }
  delete(CodEmp : number):Observable<{}>{
    return this.http.delete<Emp>(`${environment.API_URL}/api/Empleados/${CodEmp}`)
    .pipe(catchError(this.handlerError));
  }
